import { maskCardSecret } from '@/components/CardSecretModal';
...
export type CardSecretPanelTheme = 'qishui' | 'netease';
...
const CardSecretPanel: React.FC<CardSecretPanelProps> = ({ theme = 'qishui' }) => {
  ...
  let cardBody: React.ReactNode = (
    <div className={classNames(styles['secretCard'], styles['isEmpty'])} aria-label='卡密信息'>
  ...
  if (cardSecret) {
    ...
    if (isTime) {
      ...
      cardBody = (
        <div
          className={classNames(styles['secretCard'], styles['isTime'], {
            [styles['isWarn']]: stateTone === 'bad',
          })}
          aria-label='卡密信息'>
          <div className={styles['secretHead']}>
            <span className={classNames(styles['typeBadge'], styles['typeTime'])}>按时长</span>
            <div className={styles['secretHeadRight']}>
              <span className={classNames(styles['stateBadge'], styles[stateTone])}>
                {stateLabel}
              </span>
              {refreshBtn}
            </div>
          ...
          <div className={classNames(styles['stateBanner'], styles[stateTone])} role='status'>
          ...
          {expireAt && (
            <div className={styles['timeBar']} aria-hidden>
              <div
                className={classNames(styles['timeBarFill'], { [styles['isFull']]: isExpired })}
      ...
      cardBody = (
        <div
          className={classNames(styles['secretCard'], styles['isCount'], {
            [styles['isWarn']]: stateTone === 'bad',
          })}
          aria-label='卡密信息'>
          <div className={styles['secretHead']}>
            <span className={classNames(styles['typeBadge'], styles['typeCount'])}>按次数</span>
            <div className={styles['secretHeadRight']}>
              <span className={classNames(styles['stateBadge'], styles[stateTone])}>
                {stateLabel}
              </span>
          ...
          <div className={styles['progressWrap']}>
            ...
            <div
              ...
              aria-valuemax={100}>
              <div
                className={classNames(styles['progressFill'], { [styles['isFull']]: depleted })}
                style={{ width: `${percent}%` }}
              />
            </div>
          </div>
